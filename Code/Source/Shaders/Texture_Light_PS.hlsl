...
//Texture2DMS<float4, 4> baseTextureMS : register(t0);
// baseTextureMS.Sample[0][texcoord];
...
float4 main(float4 uvw : UV, float4 nrm : NORMAL, float4 col : COLOR, float4 pos : POSITION) : SV_TARGET
{
	...
	float4 dLight = dLightRatio * dLightCol * textureColor;
	...
	float4 pLight = pLightRatio * pLightCol * textureColor;
	...
	float4 sLight = spotFactor * sLightRatio * sLightCol * textureColor;

		sLight = saturate(sLight + finalSpec) * sLightDistAtten * sLightConeAtten;

		return float4(saturate(dLight + pLight + sLight).xyz, textureColor.a);
}